...
describe("useChatHandlers — channel events", () => {
  ...
  it("ChannelCreated: fetches channels, nothing else", () => {
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-1", [ev("ChannelCreated")]);
    });
  ...
  it("ChannelInvited: fetches channels + members + subscribes to new context", () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-1", [ev("ChannelInvited", {})]);
    });

    expect(mocks.fetchChannels).toHaveBeenCalledTimes(1);
    expect(mocks.fetchMembers).toHaveBeenCalledTimes(1);
    expect(mocks.subscribeToContext).toHaveBeenCalled();
  });

  it("ChannelJoined: fetches channels + members + subscribes to new context", () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-1", [ev("ChannelJoined", {})]);
    });
  ...
  it("ChannelLeft on a background context: fetches channels + members, no onLeftChannel", () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-other", [ev("ChannelLeft")]);
    });

    expect(mocks.fetchChannels).toHaveBeenCalledTimes(1);
    expect(mocks.fetchMembers).toHaveBeenCalledTimes(1);
    expect(mocks.onLeftChannel).not.toHaveBeenCalled();
  });

  it("ChannelLeft on the active channel: calls onLeftChannel", () => {
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-active", [ev("ChannelLeft")]);
    });

    expect(mocks.onLeftChannel).toHaveBeenCalledWith("ctx-active");
    expect(mocks.fetchChannels).toHaveBeenCalledTimes(1);
  });

  it("ChannelDeleted on the active channel: same behaviour as ChannelLeft", () => {
    const chat = makeActiveChat("ctx-deleted");
    activeChatRef.current = chat;
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, chat, refs),
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-deleted", [ev("ChannelDeleted")]);
    });

    expect(mocks.fetchChannels).toHaveBeenCalledTimes(1);
    expect(mocks.fetchMembers).toHaveBeenCalledTimes(1);
    expect(mocks.onLeftChannel).toHaveBeenCalledWith("ctx-deleted");
  });

  it("ChannelDeleted on a background context: no onLeftChannel", () => {
    const chat = makeActiveChat("ctx-other");
    activeChatRef.current = chat;
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, chat, refs),
...
describe("useChatHandlers — DM events", () => {
  const activeChatRef: { current: ActiveChat | null } = { current: null };
  ...
  it("DMCreated: fetches DMs + subscribes to new DM context", async () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-1", [ev("DMCreated", {})]);
    });
  ...
  it("DMDeleted: fetches DMs only", () => {
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-1", [ev("DMDeleted")]);
    });
  ...
  it("InvitationAccepted: fetches DMs", () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-1", [ev("InvitationAccepted")]);
    });
  ...
  it("NewIdentityUpdated: fetches DMs", () => {
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-1", [ev("NewIdentityUpdated")]);
    });
...
describe("useChatHandlers — message events", () => {
  const activeChatRef: { current: ActiveChat | null } = { current: null };
...
describe("useChatHandlers — full-refresh events", () => {
  ...
  it("ChatInitialized: fetches channels + DMs + members", async () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
    ...
    await act(async () => {
      result.current.handleExecutionEvents("ctx-1", [ev("ChatInitialized")]);
      await new Promise((r) => setTimeout(r, 50));
    });
...
describe("useChatHandlers — multi-event batches", () => {
  const activeChatRef: { current: ActiveChat | null } = { current: null };
  ...
  it("ChannelCreated + DMCreated in one batch: both channel and DM lists refreshed", async () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
  ...
  it("ChannelLeft + ChannelDeleted deduplicate to a single fetchChannels call", () => {
    const { refs, mocks } = makeRefs();
    const { result } = renderHook(() =>
      useChatHandlers(activeChatRef as RefObject<ActiveChat | null>, null, refs),
    );
    ...
    act(() => {
      result.current.handleExecutionEvents("ctx-other", [
        ev("ChannelLeft"),
        ev("ChannelDeleted"),
      ]);
    });

    // Actions are accumulated then executed once — one fetchChannels call
    expect(mocks.fetchChannels).toHaveBeenCalledTimes(1);
  });
